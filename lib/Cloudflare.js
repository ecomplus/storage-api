    ...
    cloudflareClient({
      ...
      .then(async ({ data }) => {
        ...
        for (let i = 0; i < pictureOptims.length; i++) {
          const { label, webp, avif } = pictureOptims[i]
          const variantUrl = variants.find(url => url.endsWith(`/${label}`))
          const Accept = (avif ? 'image/avif' : (webp ? 'image/webp' : 'image/png,image/jpeg')) + ',image/*,*/*;q=0.8'
          try {
            ...
            transformations.push({ label, webp, avif, imageBody: data })
          } catch (err) {
            logger.error(err)
          }
        }
