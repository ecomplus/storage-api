...
const runMethod = (method, params) => {
  ...
  return new Promise((resolve, reject) => {
    setTimeout(() => {
      ...
      this.s3[method](params, (err, data) => {
        if (err) {
          reject(err)
        } else {
          // success
        ...
        // log request
        const log = 'AWS.' + method + '(' + params.Bucket + ') => ' +
...
const listObjects = (bucket, prefix, marker) => {
...
module.exports = function ({ awsEndpoint, locationConstraint, accessKeyId, secretAccessKey }) {
  ...
  this.getBucketSize = getBucketSize
